...
class TemplateRepository {
    constructor() {
        this.db = new better_sqlite3_1.default('db/templates.db', { verbose: console.log });
    ...
    createPokemon(pokemon) {
        const statement = this.db.prepare("INSERT INTO templates (name, type, evolution_id, xp, xp_max, level, level_max, hp_max, capacite_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    updatePokemon(pokemonId, pokemon) {
        const statement = this.db.prepare("UPDATE templates SET name, type, evolution_id, xp, xp_max, level, level_max, hp_max, capacite_id WHERE pokemon_id=" + pokemonId.toString());
        return statement.run(pokemon.name, pokemon.type, pokemon.evolution_id, pokemon.xp, pokemon.xp_max, pokemon.level, pokemon.level_max, pokemon.hp_max, pokemon.capacite_id).lastInsertRowid;
    }
}
